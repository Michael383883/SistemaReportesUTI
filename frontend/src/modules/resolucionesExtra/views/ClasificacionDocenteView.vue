<template>
  <div class="min-h-screen bg-gray-50 p-4">
    ...
    <!-- Header -->
    <div class="flex items-center gap-3 mb-5 bg-white rounded-xl border border-gray-200 p-3.5">
      <router-link
        :to="{ name: 'clasificaciones-listado' }"
        class="flex items-center justify-center w-8 h-8 rounded-lg text-gray-400 hover:text-gray-700 hover:bg-gray-100 transition-colors flex-shrink-0"
      ...
      <div class="min-w-0 flex-1">
        <p class="text-[10px] font-semibold text-blue-500 uppercase tracking-wide">
          Archivos enlazados al docente
        </p>
        <h1 class="text-[16px] font-semibold text-gray-900 truncate leading-tight">
          {{ nombreDocente }}
        </h1>
      ...
      <!-- Buscador (escritorio) -->
      <div class="relative flex-shrink-0 w-full max-w-[220px] hidden sm:block">
        ...
        <input
          ...
          class="w-full pl-8 pr-7 py-1.5 text-[12px] border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-100 focus:border-blue-300 transition-colors"
        />
        <button
          v-if="busqueda"
          @click="busqueda = ''"
    ...
    <!-- Buscador (móvil) -->
    <div class="relative mb-4 sm:hidden" v-if="clasificaciones.length">
      <svg class="w-4 h-4 absolute left-2.5 top-1/2 -translate-y-1/2 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
        <path stroke-linecap="round" stroke-linejoin="round" d="M21 21l-4.35-4.35M17 11a6 6 0 11-12 0 6 6 0 0112 0z"/>
      </svg>
      <input
        ...
        type="text"
        placeholder="Buscar documento..."
        class="w-full pl-8 pr-7 py-2 text-[13px] bg-white border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-100 focus:border-blue-300 transition-colors"
      />
      <button
        v-if="busqueda"
        @click="busqueda = ''"
    ...
    <!-- Estados -->
    <div v-if="loading" class="text-center py-12 text-[13px] text-gray-400">
    ...
    <div v-else-if="errorMsg" class="bg-white rounded-xl border border-red-200 p-8 text-center text-[13px] text-red-500">
      {{ errorMsg }}
    </div>

    <div v-else-if="!clasificaciones.length" class="bg-white rounded-xl border border-gray-200 p-12 text-center text-[13px] text-gray-400">
    ...
    <div v-else-if="!clasificacionesFiltradas.length" class="bg-white rounded-xl border border-gray-200 p-12 text-center text-[13px] text-gray-400">
      <svg class="w-12 h-12 mx-auto text-gray-300 mb-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
        <path stroke-linecap="round" stroke-linejoin="round" d="M21 21l-4.35-4.35M17 11a6 6 0 11-12 0 6 6 0 0112 0z"/>
      </svg>
      No se encontraron documentos para "<span class="text-gray-500 font-medium">{{ busqueda }}</span>"
      <div class="mt-3">
        <button @click="busqueda = ''" class="text-[12px] text-blue-500 hover:underline">Limpiar búsqueda</button>
      </div>
    </div>
    ...
    <div v-else class="relative">
      ...
      <div class="space-y-3">
        ...
        >
          ...
          <span
            class="absolute left-[10px] top-4 w-3 h-3 rounded-full ring-4 ring-gray-50 z-10"
          ...
          <div class="bg-white rounded-xl border border-gray-200 hover:border-blue-200 transition-colors overflow-hidden">
            ...
            <!-- CABECERA -->
            <div class="flex items-center justify-between gap-2 px-3.5 py-2 bg-gray-50/50 border-b border-gray-100">
              <div class="flex items-center gap-2.5 min-w-0 flex-1">
                <!-- CATEGORIA -->
                <span class="inline-flex px-2 py-0.5 rounded-full text-[10px] font-medium flex-shrink-0" :class="badgeCategoria(c.CATEGORIA)">
                  {{ c.CATEGORIA }}
                </span>
                <!-- NIVEL (texto normal, sin colores) -->
                <span class="text-[11px] font-medium text-gray-600 flex-shrink-0">
                  {{ c.NIVEL || '—' }}
                </span>
                <span class="text-[13px] font-medium text-gray-800 truncate flex-1">
                  {{ c.DETALLE_GENERAL || 'Sin título' }}
                </span>
                <span v-if="c.FECHA_REGISTRO" class="text-[10px] text-gray-400 flex-shrink-0 hidden sm:inline">
              ...
              <div class="flex items-center gap-1.5 flex-shrink-0">
                <a
                  ...
                  target="_blank"
                  class="inline-flex items-center gap-1 px-2 py-1 text-[11px] font-medium text-blue-600 hover:bg-blue-50 rounded-lg transition-colors"
                >
                  <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                ...
                <span v-else class="text-[11px] text-gray-300 px-2">Sin PDF</span>
            ...
            <!-- CONTENIDO -->
            <div class="px-3.5 py-2 space-y-1.5">

              <div v-if="c.GESTION || c.PERIODO" class="flex items-center gap-3 text-[11px] text-gray-500">
                <span v-if="c.GESTION" class="flex items-center gap-1">
                  <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/>
                  </svg>
                  {{ c.GESTION }}
                </span>
                <span v-if="c.PERIODO" class="flex items-center gap-1">
                  <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
              ...
              <div v-if="c.OBSERVACION || c.OBSERVACION2" class="flex flex-wrap gap-1.5">
                <span v-if="c.OBSERVACION" class="inline-flex items-center gap-1 px-2 py-0.5 bg-gray-100 rounded text-[10px] text-gray-600">
                  {{ c.OBSERVACION }}
                </span>
                <span v-if="c.OBSERVACION2" class="inline-flex items-center gap-1 px-2 py-0.5 bg-gray-100 rounded text-[10px] text-gray-600">
                  {{ c.OBSERVACION2 }}
                </span>
              </div>

              <div v-if="c.materias?.length" class="flex flex-wrap items-center gap-1.5">
                <span class="text-[10px] font-medium text-gray-400 uppercase tracking-wide">Materias:</span>
                <span
                  v-for="m in c.materias"
                  :key="m.ID_DETALLE"
                  class="inline-flex items-center gap-1 px-2 py-0.5 bg-blue-50 border border-blue-100 rounded text-[11px] text-blue-700"
                >
                  {{ m.NOMBRE_MATERIA }}
                  <span v-if="m.NOTA !== null && m.NOTA !== undefined" class="text-[10px] font-semibold text-blue-500">
                    {{ m.NOTA }}
                  </span>
                </span>
              </div>
              ...
              <!-- REFERENCIAS -->
              <div v-if="c.referencias?.length" class="flex flex-wrap items-center gap-1.5">
                <span class="text-[10px] font-medium text-gray-400 uppercase tracking-wide">Referencias:</span>
                <template v-for="r in c.referencias" :key="r.ID_REF">
                  <a
                    v-if="r.ID_RESOLUCION"
                    :href="`${API_BASE}/api/resoluciones/${r.ID_RESOLUCION}/pdf`"
                    target="_blank"
                    class="inline-flex items-center gap-1 px-2 py-0.5 bg-green-50 border border-green-100 hover:bg-green-100 rounded text-[11px] text-green-700 transition-colors"
                  >
                    {{ r.NRO_REFERENCIA }}
                    <span class="text-green-500 underline underline-offset-2">Ver PDF</span>
                  </a>
                  <span
                    v-else
                    class="inline-flex items-center px-2 py-0.5 bg-gray-50 border border-gray-100 rounded text-[11px] text-gray-500"
                  >
                    {{ r.NRO_REFERENCIA }}
                  </span>
                </template>
    ...
    <div v-if="mostrarModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm" @click.self="cerrarModal">
      <div class="bg-white rounded-xl shadow-2xl max-w-md w-full mx-4 p-6 transition-all duration-200 ease-out">
        ...
        <h3 class="text-center text-[16px] font-semibold text-gray-900 mb-2">
          ¿Eliminar clasificación?
        </h3>
        <p class="text-center text-[13px] text-gray-500 mb-6">
          ...
          <br>
          <span class="text-[12px] text-red-500">Esta acción eliminará todas las materias y referencias asociadas.</span>
        </p>

        <div class="flex gap-3">
          <button
            @click="cerrarModal"
            class="flex-1 px-4 py-2 text-[13px] font-medium text-gray-600 bg-gray-100 hover:bg-gray-200 rounded-lg transition-colors"
          >
            Cancelar
          </button>
          <button
            ...
            :disabled="eliminando"
            class="flex-1 px-4 py-2 text-[13px] font-medium text-white bg-red-600 hover:bg-red-700 disabled:opacity-50 disabled:cursor-not-allowed rounded-lg transition-colors flex items-center justify-center gap-2"
          >
