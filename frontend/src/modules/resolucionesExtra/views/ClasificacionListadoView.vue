<template>
  <div class="min-h-screen bg-gray-50 p-4">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
      <div>
        <h1 class="text-[18px] font-semibold text-gray-900">Documentos de Docentes</h1>
        <p class="text-[12px] text-gray-500">Listado de documentos registrados</p>
      </div>
      <div class="flex items-center gap-2 flex-wrap">
        <!-- Botón para generar reporte Excel -->
        <button
          @click="generarReporteExcel"
          class="inline-flex items-center justify-center gap-2 px-4 py-2 bg-green-600 hover:bg-green-700 text-white text-[13px] font-medium rounded-lg transition-colors whitespace-nowrap"
        ...
        <router-link
          ...
          class="inline-flex items-center justify-center gap-2 px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white text-[13px] font-medium rounded-lg transition-colors whitespace-nowrap"
        >
          <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
    ...
    <!-- Filtros compactos -->
    <div class="bg-white rounded-xl border border-gray-200 p-3 mb-4 flex flex-wrap items-center gap-2">
      <div class="flex-1 min-w-[150px] relative">
        ...
        <input
          ...
          class="w-full pl-9 pr-3 py-1.5 text-[13px] border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
        />
      </div>
      <select
        v-model="filtros.categoria"
        @change="cargar"
        class="px-3 py-1.5 text-[13px] border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white"
      >
      ...
      <input
        ...
        type="text"
        placeholder="Gestión"
        class="w-28 px-3 py-1.5 text-[13px] border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
      />
      <button
        @click="limpiarFiltros"
        class="px-3 py-1.5 text-gray-400 hover:text-gray-600 rounded-lg transition-colors"
    ...
    <!-- Estados -->
    <div v-if="clasificacion.loading.value" class="text-center py-12 text-[13px] text-gray-400">
    ...
    <div v-else-if="clasificacion.error.value" class="bg-white rounded-xl border border-red-200 p-8 text-center text-[13px] text-red-500">
      {{ clasificacion.error.value }}
    </div>

    <div v-else-if="!listadoFiltrado.length" class="bg-white rounded-xl border border-gray-200 p-12 text-center text-[13px] text-gray-400">
    ...
    <div v-else class="bg-white rounded-xl border border-gray-200 overflow-hidden">
        ...
        <table class="w-full text-[13px]">
          <thead>
            <tr class="border-b border-gray-100 bg-gray-50/80">
              <th class="text-left font-medium text-gray-500 px-4 py-2.5 text-[11px] uppercase tracking-wider">Docente</th>
              <th class="text-left font-medium text-gray-500 px-4 py-2.5 text-[11px] uppercase tracking-wider">Categoria</th>
              <th class="text-left font-medium text-gray-500 px-4 py-2.5 text-[11px] uppercase tracking-wider">Nivel</th>
              <th class="text-left font-medium text-gray-500 px-4 py-2.5 text-[11px] uppercase tracking-wider">Gestión</th>
              <th class="text-left font-medium text-gray-500 px-4 py-2.5 text-[11px] uppercase tracking-wider">Periodo</th>
              <th class="text-left font-medium text-gray-500 px-4 py-2.5 text-[11px] uppercase tracking-wider">PDF</th>
              <th class="text-right font-medium text-gray-500 px-4 py-2.5 text-[11px] uppercase tracking-wider">Acciones</th>
            </tr>
          </thead>
          <tbody>
            <tr
            ...
            >
              ...
              <td class="px-4 py-2.5">
                <div class="flex items-center gap-2">
                  <span class="font-medium text-gray-800 text-[14px] whitespace-normal break-words">
                    {{ c.NOMBRE_DOCENTE }}
                  </span>
                  <span
                    v-if="c._totalClasificaciones > 1"
                    class="inline-flex items-center justify-center px-1.5 py-0.5 min-w-[20px] bg-gray-100 text-gray-500 text-[10px] font-semibold rounded-full flex-shrink-0"
              ...
              <!-- Categoria -->
              <td class="px-4 py-2.5">
                <span class="inline-flex px-2.5 py-0.5 rounded-full text-[10px] font-medium" :class="badgeCategoria(c.CATEGORIA)">
                  {{ c.CATEGORIA }}
                </span>
              </td>
              ...
              <!-- Gestión -->
              <td class="px-4 py-2.5 text-gray-600 text-[13px] font-medium">{{ c.GESTION || '—' }}</td>
              
              <!-- Periodo -->
              <td class="px-4 py-2.5 text-gray-600 text-[13px] font-medium">{{ c.PERIODO || '—' }}</td>
              ...
              <td class="px-4 py-2.5">
                <a
                  ...
                  target="_blank"
                  class="inline-flex items-center gap-1.5 px-2.5 py-1 bg-blue-50 hover:bg-blue-100 text-blue-600 hover:text-blue-700 text-[12px] font-medium rounded-lg transition-colors"
                >
              ...
              <td class="px-4 py-2.5 text-right">
                <div class="flex items-center justify-end gap-1.5">
                  <router-link
                    ...
                    class="inline-flex items-center gap-1.5 px-3 py-1.5 text-blue-600 hover:bg-blue-50 rounded-lg text-[12px] font-medium transition-colors"
                    title="Ver línea de tiempo"
                  >
                    <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                      <path stroke-linecap="round" stroke-linejoin="round" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/>
                    </svg>
                    <span class="hidden sm:inline">Linea de vida</span>
                  </router-link>
      ...
      <div class="px-4 py-2 bg-gray-50/80 border-t border-gray-100 text-[11px] text-gray-400 flex justify-between">
    ...
    <div v-if="mostrarModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm" @click.self="cerrarModal">
      <div class="bg-white rounded-xl shadow-2xl max-w-md w-full mx-4 p-6">
        ...
        <p class="text-center text-[13px] text-gray-500 mb-6">
          ...
          <br>
          <span v-if="itemAEliminar?._totalClasificaciones > 1" class="text-[12px] text-amber-600">
          ...
          <span v-else class="text-[12px] text-red-500">
