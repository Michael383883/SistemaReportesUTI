<template>
  <div class="min-h-screen dark:bg-[#0b1120]">
    <!-- Header -->
     ...
     <div class="w-full px-2 sm:px-2 py-2 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        ...
        <div>
          <h1 class="text-xl font-bold text-slate-1000 dark:text-white tracking-tight">Gestión de Docentes</h1>
        ...
        <div class="flex items-center gap-2">
          ...
          <span class="bg-teal-50 dark:bg-teal-500/10 text-teal-700 dark:text-teal-400 text-xs font-semibold px-2.5 py-1 rounded-full border border-teal-200 dark:border-teal-400/20">
          ...
          <div class="relative" ref="exportarDropdownRef">
            ...
            >
              ...
              >
                <p class="text-xs font-semibold text-slate-500 dark:text-slate-400 mb-2 px-0.5">Lista completa</p>
                <div class="flex items-center gap-2">
                  <!-- Ver -->
                  <button
    ...
    <!-- Filtros y búsqueda -->
    <div class="px-6 py-2.5 bg-white dark:bg-[#0b1120] border-b border-slate-100 dark:border-white/[0.08]">
      <div class="flex flex-wrap items-center gap-2">
        ...
        <div class="flex items-center gap-1.5 shrink-0">
          <select
            ...
            class="h-[38px] px-2.5 py-2 bg-slate-50 dark:bg-[#111a2e] border border-slate-200 dark:border-white/[0.08] rounded-lg text-sm text-slate-700 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-teal-500"
          ...
          <button
            ...
            class="h-[38px] px-3 rounded-lg border border-amber-200 dark:border-amber-400/20 bg-amber-50 dark:bg-amber-500/10 text-amber-700 dark:text-amber-400 text-xs font-semibold hover:bg-amber-100 dark:hover:bg-amber-500/20 transition shrink-0"
          >
            Hoy
          </button>
        </div>
        ...
        <div class="relative flex-1 min-w-64">
          ...
          <input
            ...
            class="w-full pl-9 pr-3 py-2 bg-slate-50 dark:bg-[#111a2e] border border-slate-200 dark:border-white/[0.08] rounded-lg text-sm text-slate-700 dark:text-slate-200 placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-teal-500 focus:border-transparent"
          />
        </div>
        ...
        <select
          v-model="filtroGrado"
          class="px-2.5 py-2 bg-slate-50 dark:bg-[#111a2e] border border-slate-200 dark:border-white/[0.08] rounded-lg text-sm text-slate-700 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-teal-500"
        >
        ...
        <div class="flex items-center bg-slate-100 dark:bg-white/[0.06] rounded-lg p-1 ml-auto">
          <button
            @click="vista = 'tabla'"
            :class="['px-2.5 py-1 rounded-md text-sm font-medium transition-all', vista === 'tabla' ? 'bg-white dark:bg-[#111a2e] text-slate-800 dark:text-white shadow-sm' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200']"
          >
          ...
          <button
            @click="vista = 'cards'"
            :class="['px-2.5 py-1 rounded-md text-sm font-medium transition-all', vista === 'cards' ? 'bg-white dark:bg-[#111a2e] text-slate-800 dark:text-white shadow-sm' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200']"
          >
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="p-4">
      ...
      <div v-else-if="docentesFiltrados.length === 0" class="flex flex-col items-center justify-center py-20">
        <div class="w-14 h-14 bg-slate-100 dark:bg-white/[0.06] rounded-full flex items-center justify-center mb-3">
        ...
        <p class="text-slate-600 dark:text-slate-300 font-medium text-sm">No se encontraron docentes</p>
        <p class="text-slate-400 dark:text-slate-500 text-xs mt-1">Intenta con otros filtros de búsqueda</p>
        <button @click="limpiarFiltros" class="mt-3 text-teal-600 dark:text-teal-400 text-sm font-medium hover:underline">Limpiar filtros</button>
      </div>
      ...
      <div v-else-if="vista === 'tabla'" class="bg-white dark:bg-[#0f1729] rounded-xl border border-slate-200 dark:border-white/[0.08] overflow-hidden shadow-sm">
        <div class="overflow-x-auto">
          <table class="w-full text-sm">
            ...
            <tbody class="divide-y divide-slate-100 dark:divide-white/[0.06]">
              ...
              >
                <!-- Nro -->
                <td class="px-3 py-2 text-slate-800 dark:text-slate-300 text-xs">{{ (paginaActual - 1) * porPagina + idx + 1 }}</td>

                <!-- Codigo -->
                <td class="px-3 py-2 text-slate-800 dark:text-slate-300 font-mono text-2x1">{{ docente.docente }}</td>

                <!-- Nombre -->
                <td class="px-3 py-2">
                  <p class="font-medium text-slate-800 dark:text-white leading-tight text-sm uppercase">{{ formatNombre(docente.nombre_docente) }}</p>
                </td>
                <!-- CI -->
                <td class="px-3 py-2 text-slate-800 dark:text-slate-300 font-mono text-2x1">{{ docente.ci || '—' }}</td>
                ...
                <td class="px-3 py-2">
                  <div class="flex flex-col gap-0.5">
                    ...
                    <span v-if="!docente.email && !docente.email_institucional && !docente.celular_1 && !docente.fijo_1" class="text-slate-300 dark:text-slate-600 text-xs">
                      Sin contacto
                    </span>
                  </div>
                </td>
        ...
        <!-- Paginación -->
        <div class="px-3 py-2.5 border-t border-slate-100 dark:border-white/[0.08] flex items-center justify-between">
          ...
          <div class="flex items-center gap-1">
            <button
              ...
              class="px-2.5 py-1 text-xs rounded-lg border border-slate-200 dark:border-white/[0.08] text-slate-600 dark:text-slate-300 disabled:opacity-40 disabled:cursor-not-allowed hover:bg-slate-50 dark:hover:bg-white/[0.06] transition-colors"
            >Anterior</button>
            <template v-for="p in totalPaginas" :key="p">
              <button
                v-if="Math.abs(p - paginaActual) <= 2 || p === 1 || p === totalPaginas"
                @click="paginaActual = p"
                :class="['w-7 h-7 text-xs rounded-lg transition-colors', p === paginaActual ? 'bg-teal-600 text-white' : 'border border-slate-200 dark:border-white/[0.08] text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-white/[0.06]']"
              >{{ p }}</button>
      ...
      <div v-else class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
        <div
          ...
          :key="docente.docente"
          @click="abrirDetalle(docente)"
          class="bg-white dark:bg-[#0f1729] rounded-xl border border-slate-200 dark:border-white/[0.08] p-3 hover:shadow-md hover:border-teal-300 dark:hover:border-teal-400/30 transition-all cursor-pointer group"
        >
          <div class="flex items-start gap-2.5 mb-2.5">
            <div class="w-10 h-10 rounded-xl bg-teal-50 dark:bg-teal-500/10 border border-teal-100 dark:border-teal-400/20 flex items-center justify-center flex-shrink-0">
              <i class="ti ti-user text-teal-600 dark:text-teal-400" style="font-size: 20px;" aria-hidden="true"></i>
            </div>
          ...
          <div class="space-y-1">
            ...
            <div v-if="docente.email || docente.email_institucional" class="flex items-center gap-1.5 text-xs text-slate-600 dark:text-slate-300">
            ...
            <div v-if="docente.horario_cargado" class="flex items-center gap-1.5 text-xs text-blue-600 dark:text-blue-400">
          ...
          <div class="mt-2.5 pt-2.5 border-t border-slate-100 dark:border-white/[0.08] flex items-center justify-between">
        ...
        <div class="col-span-full flex items-center justify-center gap-2 mt-1" v-if="totalPaginas > 1">
          <button @click="paginaActual--" :disabled="paginaActual === 1" class="px-3 py-1.5 text-sm rounded-lg border border-slate-200 dark:border-white/[0.08] text-slate-600 dark:text-slate-300 disabled:opacity-40 hover:bg-slate-50 dark:hover:bg-white/[0.06]">Anterior</button>
          <span class="text-sm text-slate-500 dark:text-slate-400">Página {{ paginaActual }} de {{ totalPaginas }}</span>
          <button @click="paginaActual++" :disabled="paginaActual === totalPaginas" class="px-3 py-1.5 text-sm rounded-lg border border-slate-200 dark:border-white/[0.08] text-slate-600 dark:text-slate-300 disabled:opacity-40 hover:bg-slate-50 dark:hover:bg-white/[0.06]">Siguiente</button>
        </div>
      </div>
    </div>
