...
const normalizeMateria = (m) => {
    ...
    out.plan_abrev = m.plan_abrev ?? m.PLAN_ABREV ?? ''
    out.materia = m.materia ?? m.MATERIA ?? ''
    out.compartido = m.compartido ?? m.COMPARTIDO ?? false

    // ── Datos reales de materias compartidas (GRUPOS_COMPARTIDOS) ──
    // comp: '0' = origen/padre, '1' = derivada/hija
    // orden_comparte: clave que agrupa padre + hija(s) (ej. 'M1')
    // comparte: texto descriptivo que ya arma el backend (ej. "Comparte a FIN")
    out.comp = m.comp ?? m.COMP ?? ''
    out.orden_comparte = m.orden_comparte ?? m.ORDEN_COMPARTE ?? ''
    out.comparte = m.comparte ?? m.COMPARTE ?? ''
