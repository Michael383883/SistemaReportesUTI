<template>
  <div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
    ...
    <div class="p-4 grid grid-cols-2 gap-4">
      ...
      <!--
        Tipo de título: SOLO SELECCIÓN. El catálogo se administra únicamente
        desde "Periodos académicos" → sección "Tipos de título académico".
        Aquí no se puede crear ni editar un tipo: si el usuario escribe algo
        que no está en la lista, se revierte al último valor válido al salir
        del campo (blur) o al presionar Enter.
      -->
      <div class="md:col-span-2 relative">
        <label class="text-[12px] font-medium text-gray-700">Tipo de título</label>
        <div class="relative mt-1">
          <input
            v-model="tipoTituloBusqueda"
            type="text"
            readonly
            placeholder="Todos / haz clic para elegir..."
        ...
        <ul v-if="tipoOpen" class="absolute z-10 mt-1 w-full bg-white border border-gray-200 rounded-lg shadow-lg max-h-48 overflow-auto text-[13px]">
          ...
          <li v-for="opcion in TIPOS_TITULO" :key="opcion"
              @mousedown.prevent="elegirTipo(opcion)"
              class="px-3 py-2 hover:bg-amber-50 cursor-pointer">
            {{ opcion }}
          </li>
          <li v-if="TIPOS_TITULO.length === 0" class="px-3 py-2 text-gray-400 italic">
            No hay tipos configurados. Agrégalos en "Periodos académicos".
          </li>
        </ul>
      </div>
      <div>
...
// ─── Combobox: Tipo de título — SOLO SELECCIÓN ───
// El catálogo (tipos.value) se administra exclusivamente desde
// "Periodos académicos" → "Tipos de título académico". Este componente
// solo lee la lista (cargarTipos) y nunca crea ni edita valores.
const { tipos: TIPOS_TITULO, loading: loadingTipos, cargarTipos } = useTiposTitulo()

const tipoOpen = ref(false)
const tipoTituloModel = campoMayusculas('tipo_titulo')

// Input de solo lectura: muestra el valor seleccionado, no permite escribir
// texto libre (evita crear tipos nuevos desde acá).
const tipoTituloBusqueda = computed(() => tipoTituloModel.value)
