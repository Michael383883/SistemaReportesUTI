...
<template>
  <div class="relative rounded-xl border border-slate-700 bg-slate-800 overflow-hidden">
    ...
    <div class="px-5 py-4 border-b border-slate-700 flex items-center justify-between">
      <div>
        <h3 class="text-sm font-semibold text-slate-100 m-0">2. Elegí la resolución</h3>
        <p class="text-xs text-slate-400 m-0 mt-0.5">Buscá por número y hacé click para seleccionarla</p>
      </div>
      <span
        v-if="resolucionActiva"
        class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-semibold bg-emerald-500/10 text-emerald-400 border border-emerald-500/20"
      >
    ...
    <div class="p-5 space-y-4">
      ...
      <div class="relative">
        ...
        <input
          ...
          :disabled="bloqueado"
          autocomplete="off"
          class="w-full bg-slate-900 border border-slate-700 rounded-lg pl-9 pr-9 py-2.5 text-sm text-slate-100
                 placeholder-slate-500 outline-none transition-colors
                 focus:border-amber-500 focus:ring-2 focus:ring-amber-500/20
                 disabled:opacity-50 disabled:cursor-not-allowed"
          @input="onInput"
        />
        <svg v-if="loading" class="absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-amber-400 animate-spin" fill="none" viewBox="0 0 24 24">
      ...
      <p v-if="bloqueado" class="text-xs text-amber-400/80 flex items-center gap-1.5">
      ...
      <div
        v-if="resolucionActiva"
        class="flex items-center gap-3 px-4 py-3 bg-amber-500/10 border border-amber-500/20 rounded-lg"
      >
        <div class="w-9 h-9 rounded-lg bg-amber-500/20 flex items-center justify-center shrink-0">
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="text-amber-400">
            <path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/>
            <polyline points="14 2 14 8 20 8"/>
          </svg>
        </div>
        <div class="flex-1 min-w-0">
          <p class="text-sm font-semibold text-slate-100 m-0">{{ resolucionActiva.nroResolucion }}</p>
      ...
      <!-- Lista de resultados -->
      <div v-else>
        <div v-if="loading" class="flex items-center gap-2 px-3 py-6 text-sm text-slate-500 justify-center">
        ...
        <div v-else-if="filas.length === 0" class="flex flex-col items-center gap-1.5 px-3 py-8 text-sm text-slate-500">
        ...
        <ul v-else class="list-none m-0 p-0 space-y-1.5 max-h-72 overflow-y-auto">
          <li
            v-for="r in filas"
            ...
            class="flex items-center gap-3 px-3.5 py-2.5 rounded-lg cursor-pointer transition-colors
                   bg-slate-900/40 hover:bg-slate-700/60 border border-slate-700/60"
            @click="$emit('select', r)"
          >
            ...
            <div class="flex-1 min-w-0">
              <p class="text-sm font-medium text-slate-100 m-0 truncate">{{ r.nroResolucion }}</p>
              <p class="text-xs text-slate-500 m-0 mt-0.5">{{ r.anio }} · {{ r.periodo }}</p>
            </div>
