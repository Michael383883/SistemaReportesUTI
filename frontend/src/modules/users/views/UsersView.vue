  ...
  <div>
    ...
    <div class="flex items-start justify-between mb-5">
      <div>
        ...
        <p class="text-[11px] text-gray-400 mt-0.5">Gestión de accesos por rol</p>
      </div>
      <button
        @click="openCreate"
        class="flex items-center gap-1.5 px-3 py-2 bg-navy text-white text-[11px] rounded-lg hover:bg-[#051828] transition-colors"
      >
    ...
    <transition name="fade">
      <div
        ...
        class="flex items-center gap-2 bg-red-50 border border-red-200 rounded-lg px-3 py-2 mb-4 text-[11px] text-red-700"
      >
    ...
    <div class="bg-white rounded-xl border border-gray-100 overflow-hidden">
      ...
      <div v-if="usersStore.loading && !usersStore.users.length" class="py-12 text-center">
        ...
        <p class="text-[12px] text-gray-400">Cargando usuarios...</p>
      </div>

      <table v-else class="w-full text-[11px]">
        <thead>
          <tr class="border-b border-gray-50">
            <th class="px-4 py-3 text-left text-[10px] text-gray-400 font-medium">Nombre</th>
            <th class="py-3 text-left text-[10px] text-gray-400 font-medium">Correo</th>
            <th class="py-3 text-left text-[10px] text-gray-400 font-medium">Rol</th>
            <th class="py-3 text-left text-[10px] text-gray-400 font-medium">Estado</th>
            <th class="py-3 pr-4 text-right text-[10px] text-gray-400 font-medium">Acciones</th>
          </tr>
        </thead>
        <tbody>
          <tr
          ...
          <tr v-if="!usersStore.users.length && !usersStore.loading">
            <td colspan="5" class="px-4 py-10 text-center text-[12px] text-gray-400">
